...
const { getRecommendationSignals } = require('./service')
...
exports.main = async () => {
  try {
    return await getRecommendationSignals({
      gateway: createGateway(),
      openid: cloud.getWXContext().OPENID,
      now: new Date()
    })
  } catch (error) {
    console.error('getRecommendationSignals failed', error)
    return { code: 500, message: '获取衣物使用信号失败', data: {} }
  }
}
