...
function hasKnowledgeRelevantChanges(current = {}, nextPayload = {}) {
  return !isSameString(current.name, nextPayload.name)
    || !isSameString(current.image, nextPayload.image)
...
function buildKnowledgeSyncResetFields(nextPayload = {}) {
  const hasImage = Boolean(normalizeText(nextPayload.image))
  const userTags = normalizeTagList(nextPayload.tags)
  const inferredProfile = buildInferredProfile(nextPayload)
  const mergedTags = buildMergedTags(nextPayload)
  ...
  return {
    ...
    retrieval_text: buildRetrievalText(nextPayload),
    bailian_file_id: '',
    bailian_doc_id: '',
    knowledge_doc_id: '',
    knowledge_sync_provider: 'bailian',
    knowledge_sync_status: hasImage ? 'pending' : 'skipped_no_image',
