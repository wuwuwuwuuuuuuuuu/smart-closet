...
async function run() {
  ...
  const emptySummary = await getLowCarbonSummary({
    gateway: new Gateway(),
    openid: OPENID,
    now: NOW
  })
  ...
  const noUsageGateway = new Gateway({
    clothes: [
      clothing({ _id: 'new-29', name: '29天衣物', created_at: new Date('2026-06-08T00:00:00+08:00') }),
      clothing({ _id: 'idle-30', name: '30天衣物', created_at: new Date('2026-06-07T00:00:00+08:00') }),
      clothing({ _id: 'age-missing', name: '未知创建时间', created_at: null, createdAt: null }),
      clothing({ _id: 'other-user', _openid: OTHER_OPENID, user_id: OTHER_USER_ID, name: '其他用户衣物' })
    ]
  })
  const noUsageIdle = await getIdleClothes({ gateway: noUsageGateway, openid: OPENID, now: NOW })
  assert.deepStrictEqual(noUsageIdle.data.clothes.map(item => item._id), ['idle-30'])
  ...
  assert.strictEqual(noUsageIdle.data.clothes[0].unusedDays, 30)

  const mixedGateway = new Gateway({
    users: [{ _id: USER_ID, _openid: OPENID, lowCarbonPriority: true, nickname: '保留字段' }],
    clothes: [
      clothing({ _id: 'active-29', name: '29天活跃', wearCount: 3, lastWornAt: new Date('2026-06-08T00:00:00+08:00') }),
      clothing({ _id: 'idle-used-30', name: '30天闲置', wearCount: 2, lastWornAt: new Date('2026-06-07T00:00:00+08:00') }),
      clothing({ _id: 'invalid-wear', name: '异常计数', wearCount: -5, created_at: new Date('2026-07-01T00:00:00+08:00') }),
      clothing({ _id: 'missing-worn', name: '无最近日期', wearCount: 5, lastWornAt: 'bad-date', created_at: new Date('2026-05-01T00:00:00+08:00') }),
      clothing({ _id: 'never-old', name: '长期未使用', wearCount: 0, created_at: new Date('2026-05-01T00:00:00+08:00') })
    ]
  })
  ...
  assert.deepStrictEqual(mixedIdle.data.clothes.map(item => item._id), ['never-old', 'idle-used-30'])
  ...
  const priorityMissing = await getLowCarbonPriority({
    gateway: new Gateway(),
    openid: OPENID
  })
  assert.strictEqual(priorityMissing.data.lowCarbonPriority, false)

  const invalidUpdate = await updateLowCarbonPriority({
    gateway: new Gateway(),
    openid: OPENID,
    event: { enabled: 'true' }
  })
  ...
  const missingUserUpdate = await updateLowCarbonPriority({
  ...
  const updateGateway = new Gateway({
    users: [{ _id: USER_ID, _openid: OPENID, nickname: '不能覆盖', lowCarbonPriority: false }]
  })
  const updated = await updateLowCarbonPriority({
  ...
  assert.strictEqual(updateGateway.users[0].nickname, '不能覆盖')

  const signalsDisabled = await getRecommendationSignals({
    gateway: new Gateway(),
    openid: OPENID,
    now: NOW
  })
